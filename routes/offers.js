...
router.post('/add', upload.single('offerimage'), (req, res, next) => {
        ...
        if (req.fileValidationError) {
            ...
            offer.save(function (err) {
                if (err) {
                    ...
                    var offermessage = req.body.priceoffer+' '+req.body.priceofferunit+' '+req.body.priceofferapplied+' off on '+req.body.productname+' '+req.body.productbrand+' '+req.body.pack+' Kg till '+req.body.offerenddate+' with Max offer of '+req.body.offersize+' '+req.body.offersizeunit;
    ...
    if (errors) {
        ...
        if (req.fileValidationError) {
            ...
            if(req.file) {
                offer.filepath = req.file.path;
                offer.filename = req.file.filename;
                var previousFilename = './public/uploads/offer/'+req.body.previousfilename;
                fs.stat(previousFilename, function (err, stats) {
                    if (err) {
                        console.log(`fs error one ${err}`);
                    } else {
                        console.log('file deleted successfully');
                    }
                });
